CREATE OR REPLACE PACKAGE pkg_futam_adatok IS

  PROCEDURE futam_adatok(p_szezon_ev IN NUMBER
                        ,p_futam_id  IN NUMBER);

END pkg_futam_adatok;
/
CREATE OR REPLACE PACKAGE BODY pkg_futam_adatok IS

  gc_pck_nev CONSTANT VARCHAR2(30) := 'pck_futam_adatok';

  PROCEDURE futam_adatok(p_szezon_ev IN NUMBER, p_futam_id IN NUMBER) IS
    v_futam_nev VARCHAR2(30);
    v_nyertes_pilota VARCHAR2(30);
    v_nyertes_csapat VARCHAR2(30);
    v_leggyorsabb_pilota VARCHAR2(30);
    c_proc_nev CONSTANT VARCHAR(30) := 'func_hanyszor_nyert';
